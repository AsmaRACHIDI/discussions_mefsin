...
def annotate_data_from_csv_file(input_csv_df_mefsin="/home/asma/Documents/open-data-discussions/output.csv", output_csv_file_model="models_predicted_data.csv"):
...
def annotate_a_message(title, message):
    ...
    # Combinez le titre et le message dans un DataFrame
    df = pd.DataFrame({'title': [title], 'message': [message]})

    # Effectuez l'inférence avec le modèle 1
    predictions_model1 = perform_inference(model1, tokenizer1, df)
    prediction_motif = id2label[predictions_model1[0]]

    # Effectuez l'inférence avec le modèle 2 en spécifiant is_second_preprocess=True
    predictions_model2 = perform_inference(model2, tokenizer2, df, is_second_preprocess=True)
    prediction_sous_motif = id2sslabel[predictions_model2[0]]
...
categorie, sous_categorie = annotate_a_message("Problème d'accès aux données", "Bonjour, j'essaye d'accéder aux données mais je n'y arrive pas. Merci de m'aider. Cordialement.")
print("Catégorie prédite:", categorie)
print("Sous-catégorie prédite:", sous_categorie)


#annotate_data_from_csv_file(input_csv_df_mefsin="/home/asma/Documents/open-data-discussions/output.csv", output_csv_file_model="test.csv")
#annotate_a_message("Problème d'accès aux données", "Bonjour, j'essaye d'accéder aux données me je n'y arrive pas. Merci de m'aider. Cordialement.")
