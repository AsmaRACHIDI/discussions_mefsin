import pytest
from tmp import fetch_data_from_data_gouv_api, fetch_data_from_data_eco_api, format_data
...
def test_fetch_data_from_data_gouv_api():
    data = fetch_data_from_data_gouv_api()
    assert isinstance(data, list)
    assert len(data) > 0

#def test_fetch_data_from_data_eco_api(data_eco_api_url):
    #data = fetch_data_from_data_eco_api(data_eco_api_url)
def test_fetch_data_from_data_eco_api():
    data = fetch_data_from_data_eco_api()
    assert isinstance(data, list)
    assert len(data) > 0
